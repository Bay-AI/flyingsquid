...
class LabelModel(
    _triplets.Mixin, _graphs.Mixin, _observables.Mixin, _lm_parameters.Mixin
):
    def __init__(
        self,
        m,
        ...
        if (
            ...
            for marginal in lambda_marginals:
                ...
                lambda_marginals[marginal] = JointProbabilityDistribution(
                    ...
                    [float(counts[tuple(lf_vec)]) / len(L_train) for lf_vec in lf_vecs],
                )
            ...
            for moment in lambda_moment_vals:
                if lambda_moment_basis[moment] == 0:
                    ...
                    moment_val = (
                        lambda_moment_counts[moment] / lambda_moment_basis[moment]
                    )
    ...
    def fit(
        ...
        for r_val in r_vals:
            if not self.allow_abstentions or r_val[1] == tuple(["0"]):
                ...
                if equals_one_tup[0] == "1":
                    ...
                    indices = tuple(
                        sorted([int(node.split("_")[1]) for node in equals_one_tup])
                    )
                ...
                if num_lambdas(equals_one_tup) > 0 and num_Ys(equals_one_tup) > 0:
                    ...
                    zero_indices = tuple(
                        sorted([int(node.split("_")[1]) for node in equals_zero_tup])
                    )
        ...
        if solve_method[: len("triplet")] == "triplet":
            triplets, new_moment_vals, abstention_probabilities = (
                self._triplet_method_preprocess(expectations_to_estimate, solve_method)
            )
            self.triplets = triplets
    ...
    def predict_proba(self, L_matrix, verbose=True):
        ...
        max_lambda_count = max(
            [num_lambdas(clique) for clique, marginal in self.clique_marginals]
        )
        ...
        for lambda_count in range(1, max_lambda_count + 1):
            correct_lambda_cliques = [
                (clique, marginal)
                for clique, marginal in self.clique_marginals
                if num_lambdas(clique) == lambda_count
            ]
        ...
        zero_lambda_separators = [
            (clique, marginal)
            for clique, marginal in self.separator_marginals
            if num_lambdas(clique) == 0
        ]
        ...
        for i, Y_vec in enumerate(Y_vecs):
            for j, (clique, marginal) in enumerate(zero_lambda_separators):
                ...
                A_y_sep[i, j] = Y_marginal.values ** (
                    self.separator_degrees[clique] - 1
                )

        y_probs_sep = np.prod(A_y_sep, axis=1)

        denominator_ys = np.array(
            [
                y_probs_sep,
            ]
            * L_matrix.shape[0]
        ...
        predictions[predictions.sum(axis=1) == 0] += 0.001
    ...
    def predict_proba_marginalized(self, L_matrix, verbose=False):
        ...
        return np.sum(combination_probs[:, task_indices], axis=2).reshape(
            len(combination_probs) * self.v
        )
    ...
    def estimated_accuracies(self):
        ...
        for i in range(self.m):
            ...
            Y_node = [e2 for e1, e2 in self.G.edges if e1 == lambda_node and "Y" in e2][
                0
            ]
